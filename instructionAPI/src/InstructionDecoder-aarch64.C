...
namespace Dyninst {
    ...
    */
        ...
        Expression::Ptr InstructionDecoder_aarch64::makeOptionExpression(int len, int val) {
            ...
            reg = ((optionField & 0x3) == 0x3) ? ((encoding == 31) ? aarch64::x0 : aarch64::x0) : ((encoding == 31)
                                                                                                   ? aarch64::wzr
                                                                                                   : aarch64::w0);
            if (encoding != 31)
...
#define    COMMA    ,
            ...
            if (isSIMDInsn) {
                ...
                if (encoding == 31)
                    reg = ((IS_INSN_ADDSUB_IMM(insn) || IS_INSN_ADDSUB_EXT(insn) || IS_INSN_LOGICAL_IMM(insn)) &&
                           !isPstateWritten) ? (is64Bit ? aarch64::sp : aarch64::wsp) : (is64Bit ? aarch64::x0
                                                                                                 : aarch64::wzr);
                else
        ...
        Expression::Ptr InstructionDecoder_aarch64::makeRnExpr() {
            ...
            if (isSIMDInsn && !IS_INSN_LDST(insn)) {
                        ...
                        switch (imm4) {
                            ...
                            case 0x3:
                                if (imm5 & 0x1 || imm5 & 0x2 || imm5 & 0x4) {
                                    ...
                                    reg = encoding == 31 ? aarch64::x0 : aarch64::x0;
                                }
                                break;
                            default:
                                isValid = false;
                ...
                if (!(reg == aarch64::wzr || reg == aarch64::x0))
                    reg = makeAarch64RegID(reg, encoding);
            }
            else if (isFPInsn && !((IS_INSN_FP_CONV_FIX(insn) || (IS_INSN_FP_CONV_INT(insn))) && IS_SOURCE_GP(insn))) {
                switch (_typeField) {
...
*/
        ...
        Expression::Ptr InstructionDecoder_aarch64::makeMemRefReg_Rm() {
            ...
            if (encoding == 31)//zero register
                return makeRegisterExpression(makeAarch64RegID(aarch64::x0, 0));
            else
    ...
    */
        ...
        Expression::Ptr InstructionDecoder_aarch64::makeRmExpr() {
            ...
            if (isSIMDInsn) {
                ...
                reg = is64Bit ? ((encoding == 31) ? aarch64::x0 : aarch64::x0) : ((encoding == 31) ? aarch64::wzr
                                                                                                   : aarch64::w0);
                if (encoding != 31)
                    reg = makeAarch64RegID(reg, encoding);
            }
...
// except for ld/st{s, h, w} subclass
        ...
        void InstructionDecoder_aarch64::OPRRt() {
            ...
            if (IS_INSN_BRANCHING(insn)) {
                if (encoding == 31)
                    insn_in_progress->appendOperand(makeRegisterExpression(is64Bit ? aarch64::x0 : aarch64::wzr), true,
                                                    false);
                else
                    insn_in_progress->appendOperand(
